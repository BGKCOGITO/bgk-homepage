...
## Google Ads 계정 정지 대응용 확인

이번 교체본은 `시스템 우회: 클로킹` 오탐 가능성을 줄이기 위해 랜딩페이지와 전환 코드를 정리한 버전입니다.
배포 후 Google Ads 이의신청 전에 `GOOGLE_ADS_POLICY_AUDIT.md`의 배포 후 확인 항목을 먼저 확인하세요.
